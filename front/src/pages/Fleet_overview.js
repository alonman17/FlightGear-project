...
export default function FleetOverview() {
  ...
  const getData = () => {
    ...
    axios.get("/api/liveFlights/all", { mode: "no-corsx" }).then((res) => {
      setAllairCraft(res.data);
    });
    axios.get("/api/statistics/liveFlightsCount").then((res) => {
      let live = res.data;
      console.log("live fligh tcount result : ", res.data);
      axios.get(`api/statistics/fleetSize/${year}`, { mode: "no-corsx" }).then((res) => {
        setfleetSizeArr(res.data);
        setperAircraft(live / res.data[11]);
        // setperAircraft(0.6)
      });
    });

    axios.get(`/api/statistics/aircraft/all/${month + 1}`, { mode: "no-corsx" }).then((res) => {
      const arr = res.data;
      // arr.sort((a,b) => (a.millagedone > b.millagedone) ? 1 : ((b.millagedone > a.millagedone) ? -1 : 0))
      let Obj = {};
      arr.forEach((element) => {
        Obj[element.aircraftId] += element.millagedone;
      });
